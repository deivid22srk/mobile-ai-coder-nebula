...
async function runAgentExecution(chatId, clientMessages) {
  ...
  execution.promise = (async () => {
    const MAX_TOOL_LOOPS = 500; // Safety limit
    let currentIteration = 0;

    try {
      while (currentIteration < MAX_TOOL_LOOPS && !execution.cancelled) {
        currentIteration++;
        ...
        if (toolCalls.length > 0) {
          ...
          // No tool calls: check if there are incomplete plan steps
          messages.push(assistantMessage);
          ...
          if (lastPlan) {
            try {
              ...
              if (planData.summary) {
                // Parse summary like "Plan updated: 2/5 steps completed, 1 in progress."
                const match = planData.summary.match(/(\d+)\/(\d+)/);
                if (match) {
                  const completed = parseInt(match[1]);
                  const total = parseInt(match[2]);
                  hasIncompleteSteps = completed < total;
                }
              } else {
                // Check plan array directly
                const planArr = planData.plan || [];
                hasIncompleteSteps = planArr.some(s => s.status === 'pending' || s.status === 'in_progress');
              }
            } catch (_) {}
          }

          if (hasIncompleteSteps && currentIteration < loopLimit) {
            // Auto-continue: inject instruction to keep working
            broadcastEvent('status', { content: 'Auto-continuing to complete remaining steps...' });
            messages.push({
              role: 'system',
              content: 'You have incomplete steps remaining. Continue working on them. Do not ask the user for permission - just proceed with the next step.'
            });
            continue;
          }
          // No plan or all steps complete: finished
          broadcastEvent('done', { chatId });
      ...
      // Safety limit reached (should practically never happen with 500 limit)
      if (!execution.cancelled) {
        broadcastEvent('done', { chatId });
